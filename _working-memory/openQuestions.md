...
- **Opt-in bot-evasion posture — needs BlueConic/team sign-off.** Live `wired` on Shopify requires stealth (bot-detection evasion), gated behind a per-client opt-in (default off; a prospect authorizes it on their own site). Now BUILT and gated as of 2026-07-16 (`resolveEvasionMode`, default off, enforced in the runner), but the posture (ToS, politeness, whether a lead-gen tool should offer it at all) still wants an explicit team call, like the FE sign-off above. Technical caveat surfaced by the live smoke: stealth alone doesn't mask the UA under Playwright (`uafvl=HeadlessChrome` still leaks), so Klaviyo/Attentive don't reach live `wired` without a UA override — see decisionLog + antipatterns.
