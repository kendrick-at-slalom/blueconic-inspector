...
| 0:30–1:30 | Scaffold: Playwright launches, loads URL, captures requests, emits events, async iterator. One stub provider matching nothing. | Proves the stream before any detection logic exists.           |
| 1:30–2:00 | SSE wrapper. Express, one route, ~30 lines.                                                                                    | FE can hit a real endpoint.                                    |
| 2:00–3:00 | **Meta pixel** provider, hand-written.                                                                                         | Easiest known shape. Proves the provider interface.            |
| 3:00–5:00 | **Klaviyo** provider, hand-written.                                                                                            | The actual gap and the cart-recovery payload.                  |
| 5:00–6:30 | **One exit-intent vendor** (Justuno or OptiMonk). Script presence only.                                                        | The on-site recovery story.                                    |
| 6:30–8:00 | Run against 3 real target sites. Fix what breaks.                                                                              | It will break.                                                 |
| 8:00+     | Buffer. It gets used.                                                                                                          |

**Cut order when it slips:** exit-intent vendor → SSE (fall back to in-process only, FE imports directly) → Klaviyo. **Never cut:** the schema, the event stream, one working provider.
...
4. Meta pixel provider — hand-written, proves the interface.
5. Klaviyo provider — hand-written, `present` vs. `wired` via request shape. This is the payload.
6. One exit-intent vendor if hours remain.
