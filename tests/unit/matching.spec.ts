...
describe('present-tier matching', () => {
	...
	const cases: Array<[string, string]> = [
		...
		['https://cdn.dynamicyield.com/api/123/api_dynamic.js', 'recs.recs.dynamic_yield'],
	...
	it('reports "none" for a watched vendor (a wired matcher is registered) and "unobservable" otherwise', () => {
		const klaviyo = matchRequest(req('https://static.klaviyo.com/x.js'), defaultRegistry, wiredIds)
			.find(s => s.id === 'cart.esp.klaviyo');
		const attentive = matchRequest(req('https://cdn.attn.tv/x.js'), defaultRegistry, wiredIds)
	...
	it('returns nothing for an unrelated request', () => {
		expect(matchRequest(req('https://example.com/theme.css'), defaultRegistry, wiredIds)).toHaveLength(0);
	});
});
